function [cBar,handles] = activateBrain_v2( M, vcontribs, subjstructs, range, cmapstruct, viewstruct, handles )
    ...
    if ~isempty(I),
        ...
        if ~isempty(II),
            ...
            if viewstruct.enablecortexcolor
                ...
                if ~isempty(spec)
                    nospec_roi=setdiff([1:Color_Mat.M(1).numEntries],spec);
...
%                        Color_Cindex(Color_Mat.M(1).Cortex_Color(:,1)+size(Color_Mat.M(2).vert,1),:)=Color_Mat.M(1).Cortex_Color(:,2:4); 
                ...
                hh=trisurf(M.tri, M.vert(:, 1), M.vert(:, 2), M.vert(:, 3),  'FaceVertexCData',  Color_Cindex, 'FaceColor', 'interp', 'CDataMapping', 'direct', 'linestyle', 'none','FaceAlpha',alpha);%0.1
                set(hh,'FaceLighting','phong','AmbientStrength',0.5);
            else
                if viewstruct.enablewhitematter
                    hold on;
                    load('Whitematter.mat');%% white matter surface
                    hw=trisurf(white.tri, white.vert(:, 1), white.vert(:, 2), white.vert(:, 3), 'FaceVertexCData', ixbrainc, 'CDataMapping', 'direct', 'linestyle', 'none','FaceAlpha',alpha);
                    set(hw,'FaceLighting','phong','AmbientStrength',0.5);
                else
                    
                    hc=trisurf(M.tri, M.vert(:, 1), M.vert(:, 2), M.vert(:, 3), 'FaceVertexCData', ixbrainc, 'CDataMapping', 'direct', 'linestyle', 'none','FaceAlpha',alpha);%0.1
    ...
    for k = 1 : Ss;
        ...
        if ~isempty(I),
             if handles.ElectrodeIndex==3
                switch handles.electrode_view
                    case 1 % seeg only
                        seegnum=subjstructs(k).seeg_pos;
                        
                         plotBalls(subjstructs(k).electrodes(1:seegnum,:), ballColor, ballRadius,handles);
                    case 2 % ecog only
                         seegnum=subjstructs(k).seeg_pos;
                         plotBalls(subjstructs(k).electrodes(seegnum+1:end,:), ballColor, ballRadius,handles);
                    case 3 %both
                        seegnum=subjstructs(k).seeg_pos;
                        plotBalls(subjstructs(k).electrodes(1:seegnum,:), ballColor, ballRadius,handles);
                        hold on
             ...
             plotBalls(subjstructs(k).electrodes, ballColor, ballRadius,handles);
             hold on
             end
        ...
        if ~isempty(I),
            if handles.ElectrodeIndex==3
                    ...
                    case 1 % seeg only
                        ...
                        plotBalls(subjstructs(k).trielectrodes(1:seegnum,:), ballColor_Trielectrodes, ballRadius_Trielectrodes,handles);
                    case 2 % ecog only
                         seegnum=subjstructs(k).seeg_pos;
                         plotBalls(subjstructs(k).trielectrodes(seegnum+1:end,:), ballColor_Trielectrodes, ballRadius_Trielectrodes,handles);
                    case 3 %both
                         plotBalls(subjstructs(k).trielectrodes, ballColor_Trielectrodes, ballRadius_Trielectrodes,handles);
                    end
            else
              plotBalls(subjstructs(k).trielectrodes, ballColor_Trielectrodes, ballRadius_Trielectrodes,handles);
            end
