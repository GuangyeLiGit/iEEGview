function handles = plotactivations_std(M,eleS,activations,kernelpara,sub_info,pathToSubjectDir,handles)
...
% global View;
if ~handles.CalculationFlag_Activation_std 

%%%%updated by Guangyeli @liguangye.hust@Gmail.com @USA @2018.02.01
% opengl neverselect
%Projecting the electrodes
handles.pathToStdDir=[handles.ConfigurationPath.iEEGview_directory,'/iEEGview/StdbrainModel'];
handles.normdist=kernelpara(4);
if handles.modeltype==1

  tala=projectElectrodesDepthGridStd(handles.pathToStdDir,tala,handles,'mni');  
else
  tala=projectElectrodesDepthGridStd(handles.pathToStdDir,tala,handles,'fsave');  
end
...
switch handles.modeltype
    case 1
placeToSaveFile = strcat(pathToSubjectDir, '/MATLAB/', subjectID, '_Activations_std_mni.mat');
save(placeToSaveFile, 'cmapstruct', 'cortex', 'ix', 'tala', 'vcontribs', 'viewstruct','vcontribs_norm');
    case 2
placeToSaveFile = strcat(pathToSubjectDir, '/MATLAB/', subjectID, '_Activations_std_fsave.mat');
save(placeToSaveFile, 'cmapstruct', 'cortex', 'ix', 'tala', 'vcontribs', 'viewstruct','vcontribs_norm');
end
